...
async function finalize(isRestart, channels) {
  if (Registry$AgdaModeVscode.isEmpty()) {
    Singleton$AgdaModeVscode.Panel.destroy();
    if (!isRestart) {
      Singleton$AgdaModeVscode.DebugBuffer.destroy();
    }
    return await Connection__Manager$AgdaModeVscode.disconnect(channels.log);
  }
...
function activateWithoutContext(platformDeps, subscriptions, extensionUri, globalStorageUri, memento) {
  ...
  subscribe(Vscode.workspace.onDidCloseTextDocument(function ($$document) {
            if (isAgda($$document)) {
              Registry$AgdaModeVscode.removeAndDestroy($$document).then(function () {
                    return finalize(false, channels);
                  });
              return ;
            }
            ...
            if (typeof command !== "object") {
              switch (command) {
                case "Quit" :
                    ...
                    break;
                case "Restart" :
                    await Connection__Manager$AgdaModeVscode.disconnect(channels_log);
                    await Registry$AgdaModeVscode.removeAndDestroy($$document);
