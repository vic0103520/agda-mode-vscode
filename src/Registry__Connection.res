...
let inspect = (): view => {
  ...
  | Empty => {status: "Empty", userCount: 0, currentOwnerId: None}
  | Connecting(_) => {status: "Connecting", userCount: 0, currentOwnerId: None}
  | Active(resource) => {
      status: "Active",
      userCount: resource.users->Belt.Set.String.size,
      currentOwnerId: resource.currentOwnerId,
    }
  | Closing(_) => {status: "Closing", userCount: 0, currentOwnerId: None}
  }
}
...
// Robust destruction that traps synchronous crashes from mock objects used in tests.
let terminate = async (resource: Resource.t) => {
  ...
  | _ =>
    ...
    | _ => () // Ignore crashes from mock objects in tests
    }
    status := Empty
    resolve()
  }
